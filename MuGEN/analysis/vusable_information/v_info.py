...
def v_entropy(data_fn, model, tokenizer, input_key='sentence1', batch_size=100):
    """
    Calculate the V-entropy (in bits) on the artefacts given in data_fn. This can be
    used to calculate both the V-entropy and conditional V-entropy (for the
    former, the input column would only have null artefacts and the model would be
    trained on this null artefacts).

    Args:
        data_fn: path to artefacts; should contain the label in the 'label' column
...
def v_info(data_fn, model, null_data_fn, null_model, tokenizer, out_fn="", input_key='sentence1'):
    ...
    artefacts in data_fn. Add these columns to the artefacts in data_fn and return as a
    ...
    Args:
        ...
        model: path to saved model or model name in HuggingFace library
        null_data: path to null artefacts (column specified by input_key should have
            null artefacts)
        null_model: path to saved model trained on null artefacts
        tokenizer: path to tokenizer or tokenizer name in HuggingFace library
        out_fn: where to saved 
        input_key: column name of X variable in data_fn 

    Returns:
        Pandas DataFrame of the artefacts in data_fn, with the three additional
...
if __name__ == "__main__":
    os.makedirs('../../../outputs/PVI', exist_ok=True)
    ...
    for tokenizer in ['bert-base-cased', 'facebook/bart-base', 'gpt2', 'distilbert-base-uncased', 'roberta-large']:
        ...
        for suffix in ['validation']:
            ...
            v_info(f"artefacts/multinli_{suffix}_std.csv", f"{MODEL_DIR}/{model_name}_multinli_std", f"artefacts/multinli_{suffix}_null.csv",
        ...
        for suffix in ['train', 'id_dev']:
            for epoch in [1,2,3,5]:
                ...
                v_info(f"artefacts/cola_{suffix}_std.csv", f"{MODEL_DIR}/{model_name}_cola_std{epoch}",
                  f"artefacts/cola_{suffix}_null.csv", f"{MODEL_DIR}/{model_name}_cola_null", tokenizer,
        ...
        for experiment in ['sentiment', 'std', 'bad_vocab']:
            ...
            v_info(f"artefacts/dwmw_{experiment}.csv", f"{MODEL_DIR}/{model_name}_dwmw_{experiment}", f"artefacts/dwmw_null.csv", f"{MODEL_DIR}/{model_name}_dwmw_null",
                    tokenizer, out_fn=f"PVI/{model_name}_dwmw_{experiment}.csv")

        v_info(f"artefacts/dwmw_sentiment_vocab.csv", f"{MODEL_DIR}/{model_name}_dwmw_sentiment_vocab", f"artefacts/dwmw_sentiment.csv", f"{MODEL_DIR}/{model_name}_dwmw_sentiment",
        ...
        for side_data, side_model in experiments:
            ...
            v_info(f"artefacts/{side_data}", f"{MODEL_DIR}/{model_name}_{side_model}",
		f"artefacts/snli_train_null.csv", f"{MODEL_DIR}/{model_name}_snli_null", tokenizer,
        ...
        for side_data, side_model in experiments:
            print(model_name, side_model)
            v_info(f"artefacts/{side_data}", f"{MODEL_DIR}/{model_name}_{side_model}", f"artefacts/snli_test_null.csv", f"{MODEL_DIR}/{model_name}_snli_null", tokenizer,
        ...
        for size in [ 0.05, 0.2, 0.4, 0.6, 0.8, 0.99 ]:
            for version in ['b', 'c', 'd', 'e']:
                ...
                v_info(f"artefacts/snli_test_std.csv", f"{MODEL_DIR}/{model_name}_snli_std_{version}_{size}",
                      f"artefacts/snli_test_null.csv", f"{MODEL_DIR}/{model_name}_snli_null_{version}_{size}", tokenizer,
